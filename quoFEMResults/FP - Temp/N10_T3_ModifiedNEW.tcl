...
# record nodal displacment, acceleration, and porepressure
eval "recorder Node -file displacement.out -time -dT $recDT -node $nodeList3 -dof 1 disp"
...
set listnode [list 1 [expr $nElemY(1)*2 + 1] [expr $nElemY(2) + $nElemY(1)*2 + 1] [expr $nElemY(2)*2 + $nElemY(1)*2 + 1] [expr $nElemY(2)*2 + $nElemY(1)*2 + 1 + $nElemY(3)*2]]


 # looping between the nodes    
for {set i 0} {$i < [llength $listnode]} {incr i} { 
    set dispfile [open "displacement.out" "r"]
    set z [lindex $listnode $i]
    puts $z
